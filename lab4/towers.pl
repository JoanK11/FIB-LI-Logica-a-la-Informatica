...
satVariable( villageWithTower(V) ):- village(V). % means "village V has a tower"
satVariable( villagePos(V, I, J) ):- village(V), posVillage(V, I, J).
...
writeClauses(MaxNumTowers):-
    maxNumberOfTowers(MaxNumTowers),
    towerInVillage,
    relacio,
    atMostOneTowerPerVillage,
    atLeastOnePositionWatched,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.


atMostOneTowerPerVillage:- village(V),
                           findall(towerPos(I, J), posVillage(V, I, J), Lits),
                           atMost(1, Lits), fail.
atMostOneTowerPerVillage.


towerInVillage:- position(I, J),
                 findall(V, posVillage(V, I, J), Lits),
                 length(Lits, 0),
                 writeOneClause([-towerPos(I, J)]),
                 fail.
towerInVillage.

atLeastOnePositionWatched:- village(V),
                            findall(villagePos(I, J), posVillage(V, I, J), LitsP),

                            atLeast(1, Lits), fail.
atLeastOnePositionWatched.


maxNumberOfTowers(MaxNumTowers):- findall(villageWithTower(V), village(V), Lits), atMost(MaxNumTowers, Lits), fail.
maxNumberOfTowers(_).

relacio:- village(V), findall(towerPos(I, J), posVillage(V, I, J), Lits), expressOr(villageWithTower(V), Lits), fail.
relacio.
...
costOfThisSolution(M,Cost):- findall(V, member(villageWithTower(V), M), L), sort(L, L1), length(L1, Cost), !.
