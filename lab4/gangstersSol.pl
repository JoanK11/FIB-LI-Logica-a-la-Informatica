...
satVariable( does(G,T,H) ):- available(G,H), task(T). %  means:  "gangster G does task T at hour H"     (MANDATORY)
satVariable( cons(G,H,C) ):- available(G,H), between(1, 10, C).
...
writeClauses(MaxConsecutiveHours):-
    atMostOneTaskPerHourAndGangster,
    noTwoDifferentTasksOnTwoConsecutiveHours,
    gangsterNotAvailable,
    neededGangstersPerTaskAndHour,
    atMostMaxConsecutiveHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

% No gangster can do two different tasks during the same hour.
atMostOneTaskPerHourAndGangster:- gangster(G), available(G,H), findall(does(G,T,H), task(T), Lits), atMost(1, Lits), fail.
atMostOneTaskPerHourAndGangster.

% No gangster can do two different tasks on two consecutive hours.
noTwoDifferentTasksOnTwoConsecutiveHours:- gangster(G), available(G,H1), H2 is H1+1, available(G,H2),
                                           task(T1), task(T2), T1 < T2,
                                           writeOneClause([-does(G,T1,H1), -does(G,T2,H2)]), fail.
noTwoDifferentTasksOnTwoConsecutiveHours.

% Some gangsters are not available on certain hours.
gangsterNotAvailable:- blocked(G,H), task(T), writeOneClause([-does(G,T,H)]), fail.
gangsterNotAvailable.

neededGangstersPerTaskAndHour:- needed(T,H,N), findall(does(G,T,H), available(G,H), Lits), exactly(N, Lits), fail.
neededGangstersPerTaskAndHour.

atMostMaxConsecutiveHours:- .
atMostMaxConsecutiveHours.
