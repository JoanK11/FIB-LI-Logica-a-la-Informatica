...
% VARIABLES
satVariable( sc(S, C) ):- cityStation(C, S).
satVariable( s(S) ):- station(S).
satVariable( c(C) ):- city(C).

city(C):- city(C, _).
cityStation(C, S):- city(C, L), member(S, L).
...
writeClauses(K):-
    ...
    maximumKStations(K),
    relatecWithsc,
    importantCitiesTwoStations,
    true, !.
writeClauses(_).

everyCityIsCovered:- city(C), writeOneClause([c(C)]), fail.
everyCityIsCovered.

maximumKStations(K):- findall(s(S), station(S), Lits), atMost(K, Lits).
maximumKStations(_).

relatecWithsc:- city(C), findall(s(S), cityStation(C,S), Lits), expressOr(c(C), Lits), fail.
relatecWithsc.

importantCitiesTwoStations:- importantCity(C), findall(s(S), cityStation(C,S), Lits), atLeast(2, Lits), fail.
importantCitiesTwoStations.

% Prolog predicates to be implemented:
%Given a model M, Cost is its cost:
cost(M,Cost):- findall(S, member(s(S), M), Lits), length(Lits, Cost).
...
displaySol(M):-
    findall(S, member(s(S), M), Stations),
...
main:-  symbolicOutput(1), !, numStations(N), writeOneClauses(N), halt.   % print the clauses in symbolic form and halt
