...
writeClauses(MaxNumTowers):-
    ...
    significantVillagesMustHaveTower,
...
% OK
significantVillagesMustHaveTower:- significantVillage(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), writeOneClause(Lits), fail.
significantVillagesMustHaveTower.

% OK
atMostMaxNumTowers(MaxNumTowers):- findall(v(V), village(V), Lits), atMost(MaxNumTowers, Lits).
atMostMaxNumTowers(_).

eachVillageIsWatched:- village(V), findall(towerPos(I,J), position(I, J), Lits),
                       findall((I, J), (member(towerPos(I,J), Lits), posWatchesVillage(V, I, J)), Lits2),
                       atLeast(1, Lits2), fail.
eachVillageIsWatched.

% OK
relateTowerPosWithV:- village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), expressOr(v(V), Lits), fail.
relateTowerPosWithV.

% CORRECTO
notPosVillage:- position(I,J), not(posVillage(_,I,J)), writeOneClause([-towerPos(I,J)]), fail.
notPosVillage.
...
costOfThisSolution(M,Cost):- findall(V, member(v(V), M), Lits), length(Lits, Cost).
