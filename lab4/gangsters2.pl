...
satVariable( does(G,T,H) ):- gangster(G), available(G, H), task(T).  %  means:  "gangster G does task T at hour H"     (MANDATORY)
satVariable( ghc(G, H, C) ):- gangster(G), available(G, H), hour(C).
...
writeClauses(MaxConsecutiveHours):-
    eachGangsterAtMostOneTaskPerHour,
    eachGangsterNoTwoConsecutiveTasks,
    minGangstersPerTaskAndHour,
    relacio,
    maxConsHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.


% No gangster can do two different tasks during the same hour.
eachGangsterAtMostOneTaskPerHour:- gangster(G), available(G, H),
...
eachGangsterNoTwoConsecutiveTasks:- gangster(G), available(G, H1), H2 is H1+1, available(G, H2),
                                    task(T1), task(T2), T1 \= T2,
                                    writeOneClause([-does(G, T1, H1), -does(G, T2, H2)]), fail.
eachGangsterNoTwoConsecutiveTasks.


minGangstersPerTaskAndHour:- task(T), hour(H),
                             findall(does(G, T, H), available(G, H), Lits),
                             needed(T, H, N), atLeast(N, Lits), fail.
minGangstersPerTaskAndHour.


relacio:- gangster(G), not(available(G, H1)), H2 is H1+1, available(G, H2),
          task(T), writeOneClause([-does(G, T, H2), ghc(G, H2, 1)]), fail.

relacio:- gangster(G), available(G, H1), H2 is H1+1, available(G, H2),
          task(T), writeOneClause([does(G, T, H1), -does(G, T, H2), ghc(G, H2, 1)]), fail.

relacio:- gangster(G), available(G, H1), H2 is H1+1, available(G, H2),
          task(T), hour(C1), C2 is C1+1, hour(C2),
          writeOneClause([-ghc(G, H1, C1), -does(G, T, H2), ghc(G, H2, C2)]),
          fail.
relacio.

maxConsHours(MaxConsecutiveHours):- gangster(G), available(G, H),
                                    hour(MAX), MAX > MaxConsecutiveHours, writeOneClause([-ghc(G, H, MAX)]), fail.
maxConsHours(_).
...
costOfThisSolution(M, Cost):- findall(C, member(ghc(_, _, C), M), Lits), sort(Lits, LitsS), last(LitsS, Cost).

last([X], X):- !.
last([_|L], Y):- last(L, Y).

%costOfThisSolution(M, Cost).%:- findall(G, member(does(G, _, _), M), LitsG), sort(LitsG, LitsGS),
                             % member(G, LitsGS), findall(H, member(does(G, _, H), M), LitsH), sort(LitsH, LitsHS),
                              %longitud_consecutivos(LitsHS, CostG), CostG > Cost, Cost is CostG, fail, !.

longitud_consecutivos([], 0).
longitud_consecutivos([_], 1).
longitud_consecutivos([X, Y|L], C1):- Y is X+1, longitud_consecutivos([Y|L], C2), C1 is C2+1, !.
longitud_consecutivos([_, Y|L], C):- longitud_consecutivos([Y|L], C2), max(1, C2, C).

max(X,Y,X) :- X >= Y.
max(X,Y,Y) :- X < Y.

%:- ... % maximum length of consecutive task done by some gangster
