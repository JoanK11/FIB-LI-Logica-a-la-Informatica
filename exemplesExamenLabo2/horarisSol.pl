...
eachCourseRightNumberOfDays:- courseHours(C,Hs), findall( cd(C,D), day(D), Lits ), exactly(Hs,Lits), fail.
eachCourseRightNumberOfDays.

noOverlapCoursesSameYear:- year(Y), courseYear(C1,Y), courseYear(C2,Y), C1 < C2,
                           day(D), hour(H), writeClause([-cdh(C1,D,H), -cdh(C2,D,H)]), fail.
noOverlapCoursesSameYear.

noOverlapProfs:- professor(P), courseProfessors(C1,LP1), courseProfessors(C2,LP2), C1 < C2, member(P,LP1), member(P,LP2),
                 day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail.
noOverlapProfs.

noOverlapRooms:- room(R), courseRooms(C1,LR1), courseRooms(C2,LR2), C1 < C2, member(R,LR1), member(R,LR2),
                 day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R) ]), fail.
noOverlapRooms.

relateVarsCPwithUsedProf:- courseProfessors(C,LP), member(P,LP), writeClause([-cp(C,P), usedProf(P)]), fail.
relateVarsCPwithUsedProf.
