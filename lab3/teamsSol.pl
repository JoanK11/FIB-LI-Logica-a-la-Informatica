...
incompatibleWorkers(W1,W2):- workerScore(W1, S1), workerScore(W2, S2), W1 < W2, maxScore(M), S is S1 + S2, S > M.
...
eachWexactlyOneT:- worker(W), findall(wt(W, T), team(T), List), exactly(1, List), fail.
eachWexactlyOneT.

teamSizeCorrect:- team(T), findall(wt(W, T), worker(W), List),
                  minSize(MinS), atLeast(MinS, List),
                  maxSize(MaxS), atMost(MaxS, List), fail.
teamSizeCorrect.

incompatibilities:- incompatibleWorkers(W1, W2), team(T), writeOneClause([-wt(W1,T), -wt(W2,T)]), fail.
incompatibilities.
