...
satVariable( startsNH(N, H) ):- nurse(N), hour(H). % "nurse N starts at hour H"
satVariable( worksNH(N, H) ):- nurse(N), hour(H). %          "nurse N works  at hour H"
satVariable( nurseType(N, Type) ):- nurse(N), between(1, 2, Type). % "nurse N has type T"        (if Type=1: WWWRWWW, if Type=2: WWWRRWWW)
...
% (DONE)
minNursesPerHour:- hour(H),
				   findall(worksNH(N, H), nurse(N), Lits),
				   needed(H, N), atLeast(N, Lits), fail.
minNursesPerHour.

% (DONE)
nurseCantWork:- nurseIDandBlocking(N, Start, End), hourInRange(Start, End, H),
				writeOneClause([-worksNH(N, H)]), fail.
nurseCantWork.

% (DONE)
startsOneTime:- nurse(N),
				findall(startsNH(N, H), hour(H), Lits),
				exactly(1, Lits), fail.
startsOneTime.

% (DONE)
nurseOneType:- nurse(N),
			   findall(nurseType(N, T), type(T), Lits),
			   exactly(1, Lits), fail.
nurseOneType.

restHours:- nurse(N), hour(Start), type(T), hour(H),
            not(workingHourForTypeAndStartH(T, Start, H)),
            writeOneClause([-startsNH(N, Start), -nurseType(N, T), -worksNH(N, H)]),
            fail.
restHours.

workHours:- nurse(N), hour(Start), type(T), hour(H),
            workingHourForTypeAndStartH(T, Start, H),
            writeOneClause([-startsNH(N, Start), -nurseType(N, T), worksNH(N, H)]),
            fail.
workHours.
