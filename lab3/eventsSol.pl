...
%%%%%%%  1. SAT Variables: ====================================================================
% ed(E,D) means "event E is in day D"
...
writeClauses:-  
    eachEventExactlyOneOKDay, %% Complete this!
    eachEventModeratorOK, % Cada evento tiene un moderador permitido
    eachDayMaxEvents, % There is a limit on the number of events per day
    eachModeratorMaxDays, % There is a limit on the amount of days where events with the same moderator can take place
    relacionar,
    eliminateEM,
    eliminateED,
    true,!.
...
% Cada event es fa exactament en un dia, i aquest dia és vàlid
eachEventExactlyOneOKDay:-  event(E, _, LD),
                            findall(ed(E, D), member(D, LD), Lits),
                            exactly(1, Lits), fail.
eachEventExactlyOneOKDay.

% Cada event té el seu moderador OK
eachEventModeratorOK:- event(E, LM, _),
                       findall(em(E, M), member(M, LM), Lits),
                       exactly(1, Lits), fail.
eachEventModeratorOK.

% Cada dia hi ha un màxim d'events
eachDayMaxEvents:- day(D),
                   findall(ed(E, D), event(E), Lits),
                   maxEventsPerDay(Max),
                   atMost(Max, Lits), fail.
eachDayMaxEvents.

eachModeratorMaxDays:- moderator(M),
                       findall(md(M, D), day(D), Lits),
                       maxDaysPerModerator(Max),
                       atMost(Max, Lits), fail.
eachModeratorMaxDays.

relacionar:- event(E), moderator(M), day(D),
             writeOneClause([-em(E, M), -ed(E, D), md(M, D)]), fail.
relacionar.

eliminateEM:- event(E, LM, _), moderator(M), not(member(M, LM)), writeOneClause([-em(E, M)]), fail.
eliminateEM.

eliminateED:- event(E, _, LD), day(D), not(member(D, LD)), writeOneClause([-ed(E, D)]), fail.
eliminateED.
