...
writeClauses:-  
    ...
    eliminateIncorrectTrajects,
    eachTrajectAtMostOnePerDay,
    eachTrajectAtLeastPerWeek,
    relation_bdcc_dcc,
    maxKmsPerBus,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyOneTripPerBusAndDay:- bus(B), day(D),
                             findall(bdcc(B, D, C1, C2), trip(C1-C2), Lits),
                             exactly(1, Lits), fail.
exactlyOneTripPerBusAndDay.


eliminateIncorrectTrajects:- bus(B), day(D1), D2 is D1+1, day(D2), trip(C1-C2), trip(C3-C4), C2 \= C3,
                             writeOneClause([-bdcc(B, D1, C1, C2), -bdcc(B, D2, C3, C4)]), fail.
eliminateIncorrectTrajects.


eachTrajectAtMostOnePerDay:- trip(C1-C2), day(D),
                             findall(bdcc(B, D, C1, C2), bus(B), Lits),
                             atMost(1, Lits), fail.
eachTrajectAtMostOnePerDay.


eachTrajectAtLeastPerWeek:- traject(C1, C2, MinDays, _),
                            findall(dcc(D, C1, C2), day(D), Lits),
                            atLeast(MinDays, Lits), fail.
eachTrajectAtLeastPerWeek.


relation_bdcc_dcc:- day(D), trip(C1-C2),
                    findall(bdcc(B, D, C1, C2), bus(B), Lits),
                    expressOr(dcc(D, C1, C2), Lits), fail.
relation_bdcc_dcc.

maxKmsPerBus:- bus(B), consecutiveDays(D1, D2), tooLongDist(C1-C2-C3),
