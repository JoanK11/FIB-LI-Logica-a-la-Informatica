...
noDoubleRound(R):- noDoubles(Rs), member(R, Rs).
...
% (DONE)
doubles:-
    team(T), round(R2), R1 is R2-1, round(R1),
...
% (DONE)
noDoubles:-
    noDoubleRound(R), team(T),
    writeOneClause([-double(T, R)]), fail.
noDoubles.
...
% (DONE)
sixOrSevenHomes:-
    team(T), findall(home(T, R), round(R), Lits),
    atLeast(6, Lits), atMost(7, Lits), fail.
sixOrSevenHomes.

% (DONE)
atMostOneDouble:-
    team(T), findall(double(T, R), round(R), Lits),
    atMost(1, Lits), fail.
atMostOneDouble.
