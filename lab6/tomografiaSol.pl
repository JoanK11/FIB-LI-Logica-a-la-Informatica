...
p:-	ejemplo1(RowSums, ColSums),
	...
	declareConstraints(ColSums, MatrixByCols),
...
declareConstraints([], []):- !.
declareConstraints([Sum|Sums], [X|List]):- exprSuma(X, SumaX), SumaX #= Sum, declareConstraints(Sums, List).

exprSuma([X], X).
exprSuma([X|L], Sum+X):- exprSuma(L, Sum).

matrixByRows([], _, []):- !.
matrixByRows(Vars, NumCols, [Row|MatrixByRows]):- append(Row, Vars1, Vars), length(Row, NumCols), matrixByRows(Vars1, NumCols, MatrixByRows).
